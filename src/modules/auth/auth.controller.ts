...
async function deletePublicAccountData(userId: string) {
  ...
  if (ownedDogIds.length > 0) {
    ...
    if (ownedDogCleanupError) {
      throw new AppError(
        ownedDogCleanupError.message,
        500,
        "OWNED_DOG_DATA_DELETE_FAILED"
      );
    }
    ...
    if (ownedDogsDeleteError) {
      throw new AppError(
        ownedDogsDeleteError.message,
        500,
        "OWNED_DOG_DELETE_FAILED"
      );
    }
  }
...
export const signUpWithEmail: RequestHandler = async (req, res, next) => {
  try {
...
export const signInWithEmail: RequestHandler = async (req, res, next) => {
  try {
    const { email, password } = req.body;
    const supabase = getSupabaseClient();
