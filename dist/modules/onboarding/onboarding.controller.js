...
import { findDogBreedById, listDogBreedOptions } from "../dogs/dog-breed.service.js";
import { getOnboardingOptionsBase } from "./onboarding.schemas.js";
...
function consentTimestamp(agreed) {
    return agreed ? new Date().toISOString() : null;
}
async function ensureGuardianProfile(userId) {
    ...
    if (!data) {
        throw new AppError("Guardian profile is required before this step.", 409, "GUARDIAN_PROFILE_REQUIRED");
    }
    return data;
}
export const getOnboardingOptions = async (_req, res, next) => {
    try {
...
export const getOnboardingStatus = async (req, res, next) => {
    try {
        ...
        const { data: guardianProfile, error: guardianProfileError } = await supabase
            .from("guardian_profiles")
            .select("id, display_name, location_permission_agreed, notification_permission_agreed, marketing_agreed")
            .eq("id", userId)
            .maybeSingle();
        if (guardianProfileError) {
            throw new AppError(guardianProfileError.message, 500, "GUARDIAN_PROFILE_LOOKUP_FAILED");
        }
        ...
        if (guardianRowsError) {
            throw new AppError(guardianRowsError.message, 500, "DOG_GUARDIAN_LOOKUP_FAILED");
        }
        const dogIds = (guardianRows ?? []).map((row) => row.dog_id);
        const { data: dogs, error: dogsError } = dogIds.length
            ? await supabase
                .from("dogs")
                .select("id, name, breed, birth_date, weight_kg, social_preference, personality_tags, invite_code")
                .in("id", dogIds)
            : { data: [], error: null };
        if (dogsError) {
        ...
        const dogsWithRole = (dogs ?? []).map((dog) => ({
            ...dog,
...
export const getOnboardingDebugDb = async (req, res, next) => {
    try {
        const userId = getAuthUserId(req);
        const supabase = getSupabaseAdminClient();
        const { data: guardianProfile, error: guardianProfileError } = await supabase
            .from("guardian_profiles")
            .select("id, display_name, location_permission_agreed, notification_permission_agreed, marketing_agreed, location_permission_agreed_at, notification_permission_agreed_at, marketing_agreed_at, created_at, updated_at")
            .eq("id", userId)
            .maybeSingle();
        if (guardianProfileError) {
            throw new AppError(guardianProfileError.message, 500, "GUARDIAN_PROFILE_LOOKUP_FAILED");
        ...
        const { data: dogs, error: dogsError } = dogIds.length
            ? await supabase
                .from("dogs")
                .select("id, owner_id, name, breed, birth_date, weight_kg, social_preference, personality_tags, health_notes, invite_code, created_at, updated_at")
                .in("id", dogIds)
            : { data: [], error: null };
        if (dogsError) {
            throw new AppError(dogsError.message, 500, "DOG_LOOKUP_FAILED");
        ...
        const breedInfos = await Promise.all(breedIds.map(async (breedId) => findDogBreedById(breedId)));
        res.json({
            authUser: {
                id: req.authUser?.id,
...
export const upsertGuardianProfile = async (req, res, next) => {
    try {
        const userId = getAuthUserId(req);
        const { displayName, locationPermissionAgreed, notificationPermissionAgreed, marketingAgreed } = req.body;
        const supabase = getSupabaseAdminClient();
        const { data, error } = await supabase
            .from("guardian_profiles")
            .upsert({
...
export const createDogProfile = async (req, res, next) => {
    try {
        const userId = getAuthUserId(req);
        await ensureGuardianProfile(userId);
        const { name, breed, birthDate, weightKg, socialPreference, personalityTags } = req.body;
        ...
        const { data, error } = await supabase
            ...
            .select("id, name, breed, birth_date, weight_kg, social_preference, personality_tags, invite_code, created_at")
            .single();
        if (error) {
...
export const joinDogProfile = async (req, res, next) => {
    try {
        const userId = getAuthUserId(req);
        await ensureGuardianProfile(userId);
        ...
        const { data: dog, error: dogError } = await supabase
            .from("dogs")
            .select("id, name, breed, birth_date, weight_kg, social_preference, personality_tags, invite_code, owner_id")
        ...
        if (existingGuardianError) {
            throw new AppError(existingGuardianError.message, 500, "DOG_GUARDIAN_LOOKUP_FAILED");
        }
        if (existingGuardian) {
            res.json({
                dog: {
